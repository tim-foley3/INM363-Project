...
#         self.img = Surface((SCREEN_WIDTH, SCREEN_HEIGHT))
        ...
        self.reward_list = arcade.SpriteList(use_spatial_hash=True) #TF add
        self.torch_list = arcade.SpriteList(use_spatial_hash=True) #TF add
        ...
        LightEnv1.center_window(self) #Display game window in center of pc screen
        self.clock = pygame.time.Clock()

    def load_shader(self):
        # Where is the shader file? Must be specified as a path.
        shader_file_path = Path("raycasting_shader.glsl")
...
def main():
    """Main Function"""
    window = LightEnv1(SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_TITLE)
